#!/usr/bin/env bash
# Diffusion Policy trained from scratch on blocktoblock_oracle_sim (200K episodes).
# No pretrained checkpoint needed. Image-conditioned only (no language).
# Target: 1-4 GPUs, ~8GB VRAM per GPU.
set -euo pipefail
...
# --- Output ---
JOB_NAME="diffusion_scratch_oracle"
...
# --- Launch ---
echo "=== Diffusion Policy from scratch (oracle dataset) ==="
echo "Dataset: ${DATASET_REPO}"
echo "Steps: ${STEPS}, Batch: ${BATCH_SIZE}, GPUs: ${NUM_GPUS}"
echo "Horizon: ${HORIZON}, Action steps: ${N_ACTION_STEPS}, Obs steps: ${N_OBS_STEPS}"
